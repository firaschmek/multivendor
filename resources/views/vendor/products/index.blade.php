...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            ...
            <p class="text-gray-600">إدارة كتالوج منتجاتك</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <form method="GET" class="flex flex-wrap gap-4">
            ...
            <div class="flex-1 min-w-[250px]">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="البحث في المنتجات..." class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            </div>
            ...
            <select name="stock" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                ...
                <option value="low" {{ request('stock') === 'low' ? 'selected' : '' }}>مخزون منخفض</option>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition">
                ...
                <div class="p-4">
                    ...
                    <div class="flex items-center justify-between mb-3">
                        <div class="text-lg font-bold text-blue-600">{{ number_format($product->price, 2) }} د.ت</div>
                        <div class="text-sm text-gray-600">المخزون: <span class="font-medium">{{ $product->quantity }}</span></div>
                    </div>
                    ...
                    <div class="flex gap-2">
                        ...
                        <form action="{{ route('vendor.products.toggle-status', $product) }}" method="POST" class="inline">
                            @csrf
                            <button type="submit" class="bg-gray-50 hover:bg-gray-100 text-gray-600 px-3 py-2 rounded text-sm transition" title="{{ $product->is_active ? 'إيقاف' : 'تفعيل' }}">
        ...
        @empty
            <div class="col-span-full bg-white rounded-lg shadow-md p-12 text-center">
                ...
                <h3 class="text-xl font-bold text-gray-800 mb-2">لا توجد منتجات</h3>
                <p class="text-gray-600 mb-6">ابدأ بإضافة منتجات إلى متجرك</p>
                <a href="{{ route('vendor.products.create') }}" class="inline-block bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-lg transition">
                    <i class="fas fa-plus ml-2"></i>أضف منتجك الأول
                </a>
            </div>
        @endforelse
    </div>
