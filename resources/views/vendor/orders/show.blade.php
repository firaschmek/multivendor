        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-md">
                ...
                <div class="divide-y">
                    ...
                    <div class="p-6 flex gap-4">
                        ...
                        <div class="text-right">
                            ...
                            <div class="text-sm text-gray-500">Commission: {{ number_format($item->commission_amount, 2) }} TND</div>
                            <div class="text-sm text-green-600 font-medium">Your Earn: {{ number_format($item->vendor_amount, 2) }} TND</div>
                        </div>
                    </div>
                    @endforeach
                </div>
                <div class="p-6 border-t bg-gray-50">
                    <div class="flex justify-between items-center mb-2">
                        ...
                        <span class="font-bold">{{ number_format($order->orderItems->sum('subtotal'), 2) }} TND</span>
                    </div>
                    <div class="flex justify-between items-center mb-2">
                        <span class="text-gray-600">Commission:</span>
                        <span class="text-red-600">-{{ number_format($order->orderItems->sum('commission_amount'), 2) }} TND</span>
                    </div>
                    <div class="flex justify-between items-center text-lg font-bold text-green-600 pt-2 border-t">
                        <span>Your Earnings:</span>
                        <span>{{ number_format($vendorTotal, 2) }} TND</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <form action="{{ route('vendor.orders.update-tracking', $order) }}" method="POST" class="flex gap-4 mt-4">
                    @csrf
                    <input type="text" name="tracking_number" value="{{ $order->tracking_number }}" placeholder="Tracking Number" class="flex-1 px-4 py-2 border rounded-lg">
            ...
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        <span class="px-2 py-1 text-xs rounded-full inline-block mt-1 {{ $order->payment_status == 'paid' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
