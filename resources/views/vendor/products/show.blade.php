...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <a href="{{ route('vendor.products.index') }}" class="text-blue-600 hover:text-blue-800">
            <i class="fas fa-arrow-right mr-2"></i>العودة إلى المنتجات
        </a>
        <div class="flex gap-3">
            <a href="{{ route('vendor.products.edit', $product) }}" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg">
            ...
            <form action="{{ route('vendor.products.toggle-status', $product) }}" method="POST" class="inline">
                ...
                <button type="submit" class="bg-gray-600 hover:bg-gray-700 text-white px-6 py-2 rounded-lg">
                    <i class="fas fa-{{ $product->is_active ? 'eye-slash' : 'eye' }} ml-2"></i>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <h3 class="font-bold text-gray-800 mb-4">صور المنتج</h3>
            <div class="space-y-4">
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <div class="grid grid-cols-2 gap-4 mb-6">
                    <div>
                        <label class="text-sm text-gray-600">رمز المنتج (SKU)</label>
                        <div class="font-medium">{{ $product->sku }}</div>
                    </div>
                    <div>
                        <label class="text-sm text-gray-600">الفئة</label>
                        <div class="font-medium">{{ $product->category->name }}</div>
                    </div>
                    <div>
                        <label class="text-sm text-gray-600">السعر</label>
                        <div class="font-bold text-blue-600 text-xl">{{ number_format($product->price, 2) }} د.ت</div>
                    </div>
                    <div>
                        <label class="text-sm text-gray-600">المخزون</label>
                ...
                <div class="mb-4">
                    <label class="text-sm text-gray-600">الوصف</label>
                    <p class="text-gray-800 mt-2">{{ $product->description }}</p>
                </div>

                @if($product->description_ar)
                <div>
                    <label class="text-sm text-gray-600">الوصف بالعربية</label>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div class="bg-white rounded-lg shadow-md p-4 text-center">
                    ...
                    <div class="text-sm text-gray-600">الإيرادات (د.ت)</div>
                </div>
                <div class="bg-white rounded-lg shadow-md p-4 text-center">
